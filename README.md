...
2. 시·군·구 단계의 `지역 분석 보기`는 지역 상세로, 건물 단계의 `건물 분석 보기`는 건물 상세로 이동한다.
...
4. `처음부터 초기화`는 선택된 카드와 무관하게 현재 실행 중인 시나리오를 표시하고, 그 시나리오가 만든 DEMO Case·업무·문서 초안만 제거한다. LIVE나 공유 기준자산에는 영향을 주지 않는다.
