...
if (-not [string]::IsNullOrWhiteSpace($kmaAuthKeyTrimmed)) {
    ...
    Write-Warning "KMA_AUTH_KEY is empty. Weather refresh API may fail in H2 runtime."
}
...
set CATALINA_OPTS=%CATALINA_OPTS% -Dspring.profiles.active=h2
...
try {
    ...
    # Fallback writer for environments where Set-Content intermittently fails on existing files.
    [System.IO.File]::WriteAllText($setenvBat, $setenvContent, [System.Text.Encoding]::ASCII)
}

Write-Host "[1/6] Switch DB profile -> h2"
powershell -ExecutionPolicy Bypass -File $switchScript -Profile h2 | Out-Host
