...
def test_approval_endpoints_require_authentication() -> None:
    ...
    with TestClient(app) as client:
        ...
        assert (
            client.post(
                f"/api/v1/documents/{document_id}/approval-requests",
                headers={"Idempotency-Key": "document-approval-auth"},
            ).status_code
            == 401
        )
        assert (
...
def test_approval_mutations_pass_user_version_warning_and_key(monkeypatch) -> None:
    ...
    document_request_mock = AsyncMock(return_value=requested)
    ...
    monkeypatch.setattr(
        "app.api.approvals.request_document_approval",
        document_request_mock,
    )
    try:
        with TestClient(app) as client:
            ...
            document_request_response = client.post(
                f"/api/v1/documents/{document_id}/approval-requests",
                headers={"Idempotency-Key": "document-approval-request-1"},
            )
        assert request_response.status_code == 201
        assert request_response.json()["data"] == requested
        assert decision_response.status_code == 200
        assert document_request_response.status_code == 201
        ...
        assert (
            document_request_mock.await_args.kwargs["document_draft_id"]
            == document_id
        )
    finally:
        app.dependency_overrides.clear()
...
def test_document_approval_queues_final_artifacts(monkeypatch) -> None:
    session = _session()
    app.dependency_overrides[require_csrf] = lambda: session
    approval_id = uuid4()
    ...
    decision_mock = AsyncMock(
        return_value={
            ...
            "status": "APPROVED",
            "generatedArtifactIds": [str(value) for value in artifact_ids],
        }
    )
    send_mock = Mock()
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "app.api.approvals.celery_app.send_task",
        send_mock,
    )
    try:
        with TestClient(app) as client:
            response = client.post(
                f"/api/v1/approvals/{approval_id}/decision",
                headers={"Idempotency-Key": "document-final-queue"},
                json={
                    "expectedVersion": 1,
                    "decision": "APPROVED",
                    "reason": "근거 경고를 확인하고 최종화합니다.",
                    "warningAcknowledged": True,
                },
            )
        assert response.status_code == 200
        assert send_mock.call_count == 2
        for call, artifact_id in zip(
