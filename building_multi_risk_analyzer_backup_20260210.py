...
================================================================
목적: 건물연령(A25 연령컬럼 기반) + 홍수위험 + 산사태근접위험을 결합하여
...
class BuildingMultiRiskAnalyzer:
    ...
    def _classify_landslide_risk(self, gdf: gpd.GeoDataFrame) -> None:
        ...
        boundaries = np.array([0, 50, 100, 200, 500, 99999])
        level_names = np.array(['매우위험', '위험', '주의', '관심', '안전'])
    ...
    def _analyze_flood_intersection(self, building_gdf: gpd.GeoDataFrame,
        ...
        try:
            ...
            if len(risk_indices) > 0:
                ...
                building_gdf.loc[risk_indices, '홍수등급'] = '위험'
    ...
    def _apply_fire_history_weight(self, gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        """
        전기화재이력과 겹치는 건물에 종합점수 가중치 적용 - Spatial Join 최적화 버전
        ...
        최적화 내용:
        - sjoin으로 교차 건물 일괄 추출 (iterrows 제거)
        - 불필요한 GeoDataFrame 복사 최소화
        - 통계 계산 벡터화
        """
        import gc
        ...
        try:
            ...
            # 10m 버퍼 (원본이 이미 10m라면 그대로, 아니면 버퍼 적용)
            print("  버퍼 생성 중...")
            fire_gdf_10m = fire_gdf.copy()
            fire_gdf_10m['geometry'] = fire_gdf_10m.geometry.buffer(0)  # geometry 유효성 확보
            ...
            # 1단계: 50m 범위 교차 건물 추출 (sjoin)
            print("  50m 범위 교차 분석 중...")
            joined_50m = gpd.sjoin(gdf[['_orig_idx', 'geometry']], fire_gdf_50m[['geometry']],
                                   how='inner', predicate='intersects')
            indices_50m = joined_50m['_orig_idx'].unique()
            fire_flags[indices_50m] = 1  # 50m 이내
            print(f"    50m 교차 건물: {len(indices_50m):,}개")

            del joined_50m, fire_gdf_50m
            gc.collect()
            ...
            fire_flags[indices_10m] = 2  # 10m 이내 (50m보다 우선)
            print(f"    10m 교차 건물: {len(indices_10m):,}개")

            del joined_10m, fire_gdf_10m, fire_gdf
            gc.collect()
            ...
            # 임시 컬럼 제거
            gdf.drop(columns=['_orig_idx'], inplace=True)

            # 화재배수 컬럼 생성 (벡터화)
            multipliers = np.array([1.0, 1.2, 1.5])
            gdf['화재배수'] = multipliers[fire_flags]

            # 통계 출력 (벡터화)
            counts = np.bincount(fire_flags, minlength=3)
            print(f"  10m 이내 (직접 겹침): {counts[2]:,}건 → 1.5배")
            print(f"  50m 이내 (근접): {counts[1]:,}건 → 1.2배")
            print(f"  범위 밖: {counts[0]:,}건 → 1.0배")

            # 종합점수에 화재배수 적용
            if '종합점수' in gdf.columns:
                gdf['종합점수'] = gdf['종합점수'].astype(float) * gdf['화재배수']
                print(f"  → 종합점수 = 기본점수 × 화재배수 적용 완료")
    ...
    def _apply_landuse_score(self, gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        """
        용도지역지구 위험도 점수 추가
        GeoDataFrame의 '지역' 컬럼을 기반으로 각 광역지자체별 용도지역 데이터 적용

        Args:
            gdf: 분석 결과 GeoDataFrame ('지역' 컬럼 필수)
        ...
        # 지역 컬럼 확인
        if '지역' not in gdf.columns:
            print(f"\n[용도지역] '지역' 컬럼 없음 - 분석 건너뜀")
            gdf['용도코드'] = ''
            gdf['용도명'] = ''
            gdf['용도점수'] = 0.0
            return gdf
        ...
        # GeoDataFrame에서 고유 지역 추출
        unique_regions = gdf['지역'].unique()

        print(f"\n" + "=" * 70)
        print("용도지역지구 위험도 분석")
        print("=" * 70)
        print(f"  분석 대상 지역: {', '.join(unique_regions)}")
        ...
        for region_name in unique_regions:
            landuse_folder = self.base_path / "용도지역지구" / region_name

            if not landuse_folder.exists():
                print(f"  [{region_name}] 폴더 없음 - 건너뜀")
                continue

            shp_files = list(landuse_folder.glob("*.shp"))
            if not shp_files:
                print(f"  [{region_name}] SHP 파일 없음 - 건너뜀")
                continue

            try:
                # 해당 지역 건물만 추출
                region_mask = gdf['지역'] == region_name
                region_idx = gdf[region_mask].index

                if len(region_idx) == 0:
                    continue

                print(f"  [{region_name}] {shp_files[0].name} - 건물 {len(region_idx)}개")

                # 용도지역 SHP 로드
                landuse_gdf = gpd.read_file(shp_files[0], encoding='cp949')
                landuse_gdf['UCODE'] = landuse_gdf['MNUM'].apply(extract_ucode)

                # 좌표계 통일
                if landuse_gdf.crs is None:
                    landuse_gdf = landuse_gdf.set_crs(epsg=5186)
                elif landuse_gdf.crs.to_epsg() != 5186:
                    landuse_gdf = landuse_gdf.to_crs(epsg=5186)

                # 해당 지역 건물 추출 및 좌표계 통일
                gdf_region = gdf.loc[region_idx].copy()
                if gdf_region.crs is None:
                    gdf_region = gdf_region.set_crs(epsg=5186)
                elif gdf_region.crs.to_epsg() != 5186:
                    gdf_region = gdf_region.to_crs(epsg=5186)

                # 공간 조인 (건물 중심점 기준)
                gdf_region['_centroid'] = gdf_region.geometry.centroid
                gdf_points = gdf_region.set_geometry('_centroid')

                joined = gpd.sjoin(gdf_points, landuse_gdf[['UCODE', 'geometry']],
                ...
                gdf.loc[region_idx, '용도코드'] = joined['UCODE'].fillna('').values
                gdf.loc[region_idx, '용도명'] = gdf.loc[region_idx, '용도코드'].map(name_dict).fillna('')
                gdf.loc[region_idx, '용도점수'] = gdf.loc[region_idx, '용도코드'].map(code_dict).fillna(0.0)

                matched = (gdf.loc[region_idx, '용도코드'] != '').sum()
                total_matched += matched
                print(f"      → 매칭: {matched}개")

            except Exception as e:
                print(f"  [{region_name}] 분석 오류: {e}")
    ...
    def save_results(self, gdf: gpd.GeoDataFrame, filename: str = None,
        ...
        # =====================================================================
        # 용도지역지구 위험도 점수 적용 (타임아웃 적용)
        # =====================================================================
        success, result = self._run_with_timeout(
            self._apply_landuse_score,
            args=(gdf,),
            task_name="용도지역 분석"
        )
        if success and result is not None:
            ...
            print("  → 용도지역 분석 건너뜀 (타임아웃)")
            gdf['용도코드'] = ''
            gdf['용도명'] = ''
            gdf['용도점수'] = 0.0

        # =====================================================================
        # 기본점수 저장: (노후+홍수+산사태+용도지역) 합산
        # =====================================================================
        gdf['기본점수'] = gdf['종합점수'].astype(float)
        print(f"\n기본점수 저장 완료 (노후+홍수+산사태+용도지역 합산)")

        # =====================================================================
        # 전기화재이력 가중치 적용 (타임아웃 적용)
        # =====================================================================
        success, result = self._run_with_timeout(
            self._apply_fire_history_weight,
            args=(gdf,),
            task_name="전기화재이력 분석"
        )
        if success and result is not None:
            ...
            print("  → 전기화재이력 분석 건너뜀 (타임아웃)")
            gdf['전기화재'] = 0
            gdf['화재배수'] = 1.0

        # =====================================================================
        # 최종 종합등급 재계산: (노후+홍수+산사태+용도지역)*화재배수
        # =====================================================================
        print("\n최종 종합 위험등급 계산 중...")
    ...
    def print_summary(self, gdf: gpd.GeoDataFrame):
        ...
        if '홍수등급' in gdf.columns:
            print("\n[1-2. 홍수위험도]")
            print("  (홍수위험지역 교차: 5점, 미교차: 0점)")
            flood_risk_counts = gdf['홍수등급'].value_counts()
            for level, count in flood_risk_counts.items():
                pct = 100 * count / len(gdf)
                score = 5 if level == '위험' else 0
                print(f"    {level} ({score}점): {count:,}개 ({pct:.1f}%)")

            flood_buildings = (gdf['홍수점수'] > 0).sum()
            print(f"\n  홍수위험지역 교차 건물: {flood_buildings:,}개 ({100*flood_buildings/len(gdf):.1f}%)")
        ...
        if '전기화재' in gdf.columns:
            ...
            print(f"  10m 이내 (1.5배): {fire_10m:,}개 ({100*fire_10m/len(gdf):.1f}%)")
            print(f"  50m 이내 (1.2배): {fire_50m:,}개 ({100*fire_50m/len(gdf):.1f}%)")
            print(f"  범위 밖 (1.0배): {fire_none:,}개 ({100*fire_none/len(gdf):.1f}%)")

            if '화재배수' in gdf.columns:
                avg_mult = gdf['화재배수'].mean()
                print(f"\n  평균 화재배수: {avg_mult:.3f}배")
        ...
        print(f"    홍수위험:     {avg_flood_score:.2f}점")
        ...
        if '화재배수' in gdf.columns:
            avg_fire_mult = gdf['화재배수'].mean()
            fire_applied = (gdf['화재배수'] > 1.0).sum()
            print(f"    전기화재배수: 평균 {avg_fire_mult:.2f}배 (적용 건물: {fire_applied:,}개)")

        print("\n[위험요소별 고위험 건물 수]")
        high_age = (gdf['노후점수'] >= 5).sum()
        high_flood = (gdf['홍수점수'] == 5).sum()
        high_landslide = (gdf['산사태점수'] >= 3).sum()
        print(f"    노후 5점 이상:     {high_age:,}개 ({100*high_age/len(gdf):.1f}%)")
        print(f"    홍수 5점 (교차):   {high_flood:,}개 ({100*high_flood/len(gdf):.1f}%)")
        print(f"    산사태 3점 이상:   {high_landslide:,}개 ({100*high_landslide/len(gdf):.1f}%)")
        ...
        # 기본점수 vs 최종점수 비교
        if '기본점수' in gdf.columns:
            avg_base = gdf['기본점수'].mean()
            avg_final = gdf['종합점수'].mean()
            print(f"\n  [점수 비교]")
            print(f"    평균 기본점수 (합산): {avg_base:.2f}점")
            print(f"    평균 종합점수 (배수적용): {avg_final:.2f}점")
...
def merge_results_mode():
    ...
    print(f"\n발견된 SHP 파일: {len(shp_files_to_merge)}개")
    ...
    if '홍수점수' in combined_gdf.columns:
        ...
        flood_buildings = (combined_gdf['홍수점수'] > 0).sum()
        no_flood = len(combined_gdf) - flood_buildings
        print(f"    홍수위험지역 교차: {flood_buildings:,}개 ({100*flood_buildings/len(combined_gdf):.1f}%)")
        print(f"    미교차: {no_flood:,}개 ({100*no_flood/len(combined_gdf):.1f}%)")
    ...
    # 1-5. 전기화재이력 위험도
    if '전기화재' in combined_gdf.columns:
        print("\n[1-5. 전기화재이력 위험도]")
        fire_10m = (combined_gdf['전기화재'] == 2).sum()
        fire_50m = (combined_gdf['전기화재'] == 1).sum()
        fire_none = (combined_gdf['전기화재'] == 0).sum()

        print(f"    10m 이내 (1.5배): {fire_10m:,}개 ({100*fire_10m/len(combined_gdf):.1f}%)")
        print(f"    50m 이내 (1.2배): {fire_50m:,}개 ({100*fire_50m/len(combined_gdf):.1f}%)")
        print(f"    범위 밖 (1.0배): {fire_none:,}개 ({100*fire_none/len(combined_gdf):.1f}%)")

        if '화재배수' in combined_gdf.columns:
            avg_mult = combined_gdf['화재배수'].mean()
            print(f"\n    평균 화재배수: {avg_mult:.3f}배")
    ...
    if '홍수점수' in combined_gdf.columns:
        print(f"    홍수위험:     {combined_gdf['홍수점수'].mean():.2f}점")
    if '산사태점수' in combined_gdf.columns:
        print(f"    산사태위험:   {combined_gdf['산사태점수'].mean():.2f}점")
    if '용도점수' in combined_gdf.columns:
        print(f"    용도지역위험: {combined_gdf['용도점수'].mean():.2f}점")
    if '화재배수' in combined_gdf.columns:
        fire_applied = (combined_gdf['화재배수'] > 1.0).sum()
        print(f"    전기화재배수: 평균 {combined_gdf['화재배수'].mean():.2f}배 (적용 건물: {fire_applied:,}개)")

    print("\n[위험요소별 고위험 건물 수]")
    if '노후점수' in combined_gdf.columns:
        high_age = (combined_gdf['노후점수'] >= 5).sum()
        print(f"    노후 5점 이상:     {high_age:,}개 ({100*high_age/len(combined_gdf):.1f}%)")
    if '홍수점수' in combined_gdf.columns:
        high_flood = (combined_gdf['홍수점수'] == 5).sum()
        print(f"    홍수 5점 (교차):   {high_flood:,}개 ({100*high_flood/len(combined_gdf):.1f}%)")
    if '산사태점수' in combined_gdf.columns:
        high_landslide = (combined_gdf['산사태점수'] >= 3).sum()
        print(f"    산사태 3점 이상:   {high_landslide:,}개 ({100*high_landslide/len(combined_gdf):.1f}%)")
    if '용도점수' in combined_gdf.columns:
    ...
    # 기본점수 vs 최종점수 비교
    if '기본점수' in combined_gdf.columns and '종합점수' in combined_gdf.columns:
        avg_base = combined_gdf['기본점수'].mean()
...
def interactive_mode():
    ...
    if len(all_results) > 0:
        ...
        if '홍수점수' in combined_for_stats.columns:
            flood_count = (combined_for_stats['홍수점수'] == 5).sum()
            print(f"\n[홍수위험 현황]")
            print(f"  • 홍수위험지역 교차 건물: {flood_count:,}개 ({100*flood_count/len(combined_for_stats):.1f}%)")

        if '산사태점수' in combined_for_stats.columns:
            landslide_high = (combined_for_stats['산사태점수'] >= 3).sum()
            print(f"\n[산사태위험 현황]")
            print(f"  • 산사태 고위험 (3점 이상): {landslide_high:,}개 ({100*landslide_high/len(combined_for_stats):.1f}%)")
